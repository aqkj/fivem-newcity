 ...
 * 事件
 */
onNet('mnds:setPosition', (position: IPosition) => {
  setPosition(position)
})
onNet('mnds:playerLoaded', (playerData: IPlayerData) => {
...
on('baseevents:onPlayerDied', (killerType: number, deathCoords: number[]) => {
  onPlayerDied(killerType, deathCoords)
})
on('baseevents:onPlayerKilled', (killerType: number, deathCoords: number[]) => {
